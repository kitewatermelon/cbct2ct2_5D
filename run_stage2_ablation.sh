...
# Ablation 1: backbone(unet) × in_channels(1,3,5,7,9) @ cpr4_img128
# Ablation 2: backbone(unet) × compress_ratio(2,8)     @ n5

set -e
mkdir -p logs

DATA=../s2025
CKPT=checkpoints_임베딩1/stage1_vqvae
...
PYTHONPATH=. python stage2_vdm.py --batch_size 8 \
  --backbone unet --in_channels 5 --compress_ratio 4 \
  --ct_ckpt   ${CKPT}/1_vqvae_ct_n5_cpr4_img128/best.pt \
  --cbct_ckpt ${CKPT}/1_vqvae_cbct_n5_cpr4_img128/best.pt \
  --exp_name 1_vdm_unet_n5_cpr4_img128 --device 0 \
  ${COMMON} > logs/stage2_unet_n5_cpr4.log 2>&1 &